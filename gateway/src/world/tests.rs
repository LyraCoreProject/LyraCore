        ...
        match op {
            ...
            realm_op::GUILD_CHAT => {
                ...
                for member in g.members_of(guild_id) {
                    g.events
                        .push((member, event_kind::GUILD_CHAT, actor_guid, text.clone()));
                }
                Ok(())
            }
...
fn messagechat_guild_from_a_member_relays_the_line() {
    ...
    assert_eq!(
        store
            ...
            .ops
            .iter()
            .find(|(op, ..)| *op == lyracore_shared::guild::realm_op::GUILD_CHAT),
        Some(&(
...
struct FakeGuild {
    ...
    /// `game_guild_event`'s rows, which is what a broadcast IS before a relay renders it — the only
    /// way an invite popup or a `Joined` line ever reaches a client.
