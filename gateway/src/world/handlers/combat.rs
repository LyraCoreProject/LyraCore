...
pub(crate) fn handle_combat<St: WorldStore + ?Sized>(
    ...
    match msg {
        ...
        // The one cast route the cast seam has not taken over yet: ranged auto-repeat. The seam
        // classifies every CMSG_CAST_SPELL first and passes this through, so every other cast
        // (ordinary, enchant, disenchant, fishing, lock opening) never arrives here.
