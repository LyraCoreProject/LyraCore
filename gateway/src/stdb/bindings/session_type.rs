// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct Session {
    pub account_id: u64,
    pub session_key: Vec<u8>,
    pub identity: Option<__sdk::Identity>,
    pub created_at: __sdk::Timestamp,
    pub expires_at: __sdk::Timestamp,
}

impl __sdk::InModule for Session {
    type Module = super::RemoteModule;
}

/// Column accessor struct for the table `Session`.
///
/// Provides typed access to columns for query building.
pub struct SessionCols {
    pub account_id: __sdk::__query_builder::Col<Session, u64>,
    pub session_key: __sdk::__query_builder::Col<Session, Vec<u8>>,
    pub identity: __sdk::__query_builder::Col<Session, Option<__sdk::Identity>>,
    pub created_at: __sdk::__query_builder::Col<Session, __sdk::Timestamp>,
    pub expires_at: __sdk::__query_builder::Col<Session, __sdk::Timestamp>,
}

impl __sdk::__query_builder::HasCols for Session {
    type Cols = SessionCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        SessionCols {
            account_id: __sdk::__query_builder::Col::new(table_name, "account_id"),
            session_key: __sdk::__query_builder::Col::new(table_name, "session_key"),
            identity: __sdk::__query_builder::Col::new(table_name, "identity"),
            created_at: __sdk::__query_builder::Col::new(table_name, "created_at"),
            expires_at: __sdk::__query_builder::Col::new(table_name, "expires_at"),
        }
    }
}

/// Indexed column accessor struct for the table `Session`.
///
/// Provides typed access to indexed columns for query building.
pub struct SessionIxCols {
    pub account_id: __sdk::__query_builder::IxCol<Session, u64>,
}

impl __sdk::__query_builder::HasIxCols for Session {
    type IxCols = SessionIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        SessionIxCols {
            account_id: __sdk::__query_builder::IxCol::new(table_name, "account_id"),
        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for Session {}
