...
/// Armor subclass constants (item class 4) used by the proficiency gate.
pub mod armor_subclass {
    pub const MISC: u8 = 0; // rings, necks, trinkets, cloaks, shirts, tabards — no class restriction
    pub const CLOTH: u8 = 1; // universally equippable by all classes
...
/// Weapon subclass constants (item class 2) used by the proficiency gate.
...
/// Whether a player of `player_class` may equip an item with `item_class` / `item_subclass`.
/// This is the class proficiency gate — distinct from the slot-shape gate (`can_equip_into`)
/// and the level gate (`meets_required_level`). Pure — unit-tested.
///
/// Only item class 2 (Weapon) and class 4 (Armor) have per-class subclass restrictions in
/// vanilla 1.12; everything else (consumables, quest items, etc.) is always allowed. Armor
/// subclass 0 (Misc: rings/necks/trinkets/cloaks/shirts/tabards) and subclass 1 (Cloth) are
/// universally equippable — every class can wear cloth. The remaining armor tiers and all
/// weapon types are keyed on `player_class` per the vanilla proficiency table. An unknown class
/// (creatures, 0) fails closed for restricted types. [reference]
pub fn can_equip_proficiency(player_class: u8, item_class: u8, item_subclass: u8) -> bool {
    match item_class {
        ...
        _ => true, // consumables, trade goods, quest items, etc. have no class restriction
    }
}

/// Armor proficiency check: given the player's class and the item's armor subclass, can they
/// equip it? Cloth (1) and Misc (0) are allowed for all. Everything above cloth gates by class.
fn armor_proficiency(player_class: u8, subclass: u8) -> bool {
    use armor_subclass as a;
    // Subclass 0 (misc accessories) and 1 (cloth) — universally equippable.
    if subclass == a::MISC || subclass == a::CLOTH {
        return true;
    }
    match player_class {
        // Warrior + Paladin: leather, mail, plate, shield.
        1 | 2 => matches!(subclass, a::LEATHER | a::MAIL | a::PLATE | a::SHIELD),
        // Hunter: leather, mail (mail is L1-trainable in vanilla 1.12; allowed from the start).
        3 => matches!(subclass, a::LEATHER | a::MAIL),
        // Rogue: leather only.
        4 => subclass == a::LEATHER,
        // Priest: cloth only (already returned true above for cloth/misc).
        5 => false,
        // Shaman: leather, mail, shield.
        7 => matches!(subclass, a::LEATHER | a::MAIL | a::SHIELD),
        // Mage, Warlock: cloth only.
        8 | 9 => false,
        // Druid: leather only.
        11 => subclass == a::LEATHER,
        // Unknown / creature class: fail closed.
        _ => false,
    }
}

/// Weapon proficiency check: given the player's class and the weapon subclass, can they equip
/// it? The most critical restriction enforced here is wands — only Mage, Warlock, Priest can
/// use them; other classes cannot. Fishing poles are equippable by any class (they occupy the
/// ranged slot in vanilla). All other weapon-type restrictions (e.g. Priest can't use axes)
/// are enforced here too per the vanilla 1.12 proficiency table. [reference]
fn weapon_proficiency(player_class: u8, subclass: u8) -> bool {
    use weapon_subclass as w;
    // Fishing poles: equippable by any class (no combat use, just the ranged slot).
    if subclass == w::FISHING_POLE {
        return true;
    }
    match player_class {
        // Warrior: all weapon types except wands.
        1 => subclass != w::WAND,
        // Paladin: 1H/2H axes, maces, polearms, swords (no bows/guns/daggers/staves/wands/fist/thrown).
        2 => matches!(
            subclass,
            w::AXE_1H
                | w::AXE_2H
        ...
        // Hunter: axes (1H/2H), bows, crossbows, daggers, fist, guns, polearms, staves, swords (1H/2H), thrown.
        3 => matches!(
            subclass,
            w::AXE_1H
                | w::AXE_2H
        ...
        // Rogue: bows, crossbows, daggers, fist, guns, 1H maces, 1H swords, thrown.
        4 => matches!(
            subclass,
            w::BOW
                | w::CROSSBOW
        ...
        // Priest: daggers, maces (1H), staves, wands.
        5 => matches!(subclass, w::DAGGER | w::MACE_1H | w::STAFF | w::WAND),
        // Shaman: axes (1H/2H), daggers, fist, maces (1H/2H), staves.
        7 => matches!(
            subclass,
            w::AXE_1H | w::AXE_2H | w::DAGGER | w::FIST | w::MACE_1H | w::MACE_2H | w::STAFF
        ),
        // Mage: daggers, staves, 1H swords, wands.
        8 => matches!(subclass, w::DAGGER | w::STAFF | w::SWORD_1H | w::WAND),
        // Warlock: daggers, staves, 1H swords, wands.
        9 => matches!(subclass, w::DAGGER | w::STAFF | w::SWORD_1H | w::WAND),
        // Druid: daggers, fist, 1H/2H maces, staves.
        11 => matches!(
            subclass,
            w::DAGGER | w::FIST | w::MACE_1H | w::MACE_2H | w::STAFF
        ),
        // Unknown / creature class: fail closed.
        _ => false,
    }
