//! Targeting + effect dispatch + cast resolution — `select_targets` / `aura_apply` / `apply_effect`
//! and the cast core (`resolve_cast_at` / `resolve_cast` / `begin_cast`). `mod.rs` re-exports the
//! `pub(crate)` cast entry points (`resolve_cast_at`/`resolve_cast`/`begin_cast`) so the debug + creature
//! callers can reach them; `select_targets`/`aura_apply`/`apply_effect` stay private (used only within
//! this file).
...
// Tables, taxonomy consts, math helpers, and the control predicates are all re-exported by `mod.rs`.
use super::*;
...
fn recipe_reagents(ctx: &ReducerContext, spell_id: u32) -> Vec<(u32, u32)> {
    ctx.db
        .game_spell_reagent()
        .by_spell()
        .filter(&spell_id)
...
fn aura_apply(
    ...
    // stamped later, at REMOVAL (`scheduler::tick_auras`'s expiry pass / `math::dispel_target`).
    ...
    if e.kind == A_SEAL {
        let other_seal_spell_ids: std::collections::HashSet<u32> = auras
            ...
            .map(|a| a.spell_id)
            .collect();
        let others: Vec<u64> = auras
            .by_target()
            .filter(&target_guid)
            .filter(|a| other_seal_spell_ids.contains(&a.spell_id))
            .map(|a| a.id)
            .collect();
        for id in others {
            auras.id().delete(id);
    ...
    // resolve the OTHER rank's spell_id(s) sharing this name, then delete every aura row of that
    // spell_id (all its effects, not just one) — vanilla allows only one rank of a buff active at a
    // time. Name-keyed is the honest slice available today; the real rank-chain data (`spell_chain`) is
    // future work (work-item 102 tracks importing it) and will refine this later (a
    // same-named-but-unrelated pair would incorrectly collide under this heuristic — acceptable for now,
    // there is no such collision in the seed data).
    ...
    if !hdr.name.is_empty() {
        ...
        let other_rank_ids: std::collections::HashSet<u32> = auras
            ...
            .collect();
        if !other_rank_ids.is_empty() {
            let displaced: Vec<u64> = auras
                .by_target()
                .filter(&target_guid)
                .filter(|a| other_rank_ids.contains(&a.spell_id))
                .map(|a| a.id)
                .collect();
            for id in displaced {
                auras.id().delete(id);
    ...
    if let Some(mut a) = existing {
        ...
        let Some(slot) = pick_aura_slot(
            ...
            ctx.db.game_spell_cast_event().insert(SpellCastEvent {
                ...
                is_interrupted: true,
                cooldown_ms: 0,
                delay_ms: 0, // aura-slot overflow is an interrupt signal, not a pushback
                healed: 0,
                is_proc_log: false,
                swing_hit_info: 0,
                ...
                map_id: crate::helpers::grid_of(ctx, caster_guid).0,
                instance_id: crate::helpers::grid_of(ctx, caster_guid).1,
                grid_x: crate::helpers::grid_of(ctx, caster_guid).2,
                grid_y: crate::helpers::grid_of(ctx, caster_guid).3,
            });
            return;
        };
        auras.insert(Aura {
...
pub(crate) fn resolve_cast_at(
    ...
    let mut caster = ctx
        ...
        .guid()
        .find(caster_guid)
        .ok_or_else(|| format!("caster {caster_guid} not in world"))?;
    ...
    // Load the spell's effects ONCE — used by both the faction gate (target-class scan) and the effect
    // loop below. Sorted by effect_index for deterministic order, EXCEPT E_INTERRUPT resolves FIRST: a damage
    // effect at a lower index runs break_auras_on_damage→interrupt_cast, which clears the target's pending cast
    // before a later E_INTERRUPT could read its school for the lockout (Kick/Earth Shock carry
    // E_DAMAGE at index 1, E_INTERRUPT at index 2). Resolving the interrupt first makes the school-lockout
    // order-independent, so a re-import that restores the natural effect order can't silently regress it.
    ...
    if target_guid != caster_guid {
        ...
        if hdr.cast_flags & SPELL_ATTR_REQ_BEHIND != 0 {
            let target = ctx
                ...
                .guid()
                .find(target_guid)
                .ok_or_else(|| format!("spell {spell_id} target {target_guid} not in world"))?;
    ...
    if !queued_next_swing {
        ctx.db.game_spell_cast_event().insert(SpellCastEvent {
            ...
            // A cast-GO/completion is not an interrupt signal.
            ...
            delay_ms: 0, // a cast-GO/completion is not a pushback signal
            is_proc_log: false,
            swing_hit_info: 0,
            client_initiated,
    ...
    if caster_is_player && cast_hits_enemy {
        crate::spell::math::arm_spell_retaliation(ctx, caster_guid, target_guid);
    }
...
pub(crate) fn begin_cast(
    ...
    ctx.db.game_spell_cast_event().insert(SpellCastEvent {
        ...
        // A cast-START is never a completion and carries no damage (the GO event at resolve does both).
        // Explicit for clarity though the START takes the cast_time_ms>0 gateway branch regardless.
        is_completion: false,
        damage: 0,
        school: 0,
        // A START carries no damage breakdown either — the GO event at resolve sets these.
        is_crit: false,
        resisted: 0,
        absorbed: 0,
        // A cast-START is not an interrupt signal.
        is_interrupted: false,
        cooldown_ms: 0, // the cooldown starts at the GO (completion), not the begin-START
        delay_ms: 0,    // a cast-START is not a pushback signal
        healed: 0,
        is_proc_log: false,
        swing_hit_info: 0,
        client_initiated: false,
...
pub(crate) fn interrupt_cast(ctx: &ReducerContext, caster_guid: u64) -> bool {
    ...
    for (id, spell_id) in hits {
        ...
        ctx.db.game_spell_cast_event().insert(SpellCastEvent {
            ...
            absorbed: 0,
            is_interrupted: true,
            cooldown_ms: 0, // an interrupt signal carries no cooldown
            delay_ms: 0,    // an interrupt signal is not a pushback
            healed: 0,
            is_proc_log: false,
            swing_hit_info: 0,
            client_initiated: false,
...
mod tests {
    use super::{
        faction_target_violation, pick_aura_slot, should_pushback, CAST_PUSHBACK_MAX,
        CAST_PUSHBACK_MS,
    };
    ...
    fn aura_slot_is_per_spell_so_a_multi_effect_buff_is_one_icon() {
        ...
        assert_eq!(pick_aura_slot(&full, 999, false, 3, 4), None);
        // …but a sibling effect of an ALREADY-present spell still resolves even when "full" (shares its slot).
        assert_eq!(pick_aura_slot(&full, 101, false, 3, 4), Some(1));
    }
    ...
    fn sixteen_debuffs_full_refuses_the_seventeenth_leaving_the_sixteen_untouched() {
        ...
        assert_eq!(pick_aura_slot(&debuffs_full, 9999, true, 32, 48), None);
        ...
        for &(slot, spell_id) in &debuffs_full {
            assert_eq!(
                pick_aura_slot(&debuffs_full, spell_id, true, 32, 48),
                Some(slot)
            );
        }
    }
    ...
    fn faction_gate_blocks_only_the_two_exploit_cases() {
        ...
        assert_eq!(
            faction_target_violation(true, false, true, false),
            Some("enemies")
        ); // offensive on a friendly
        assert_eq!(
            faction_target_violation(false, true, false, true),
            Some("allies")
        ); // helpful on a hostile
           // Legitimate casts stay allowed:
        assert_eq!(faction_target_violation(true, false, false, true), None); // offensive on a hostile
        assert_eq!(faction_target_violation(false, true, true, false), None); // helpful on a friendly
                                                                              // NEUTRAL (neither friendly nor hostile) stays both attackable AND buffable:
        assert_eq!(faction_target_violation(true, false, false, false), None);
        assert_eq!(faction_target_violation(false, true, false, false), None);
        // A spell that targets neither (T_SELF / T_TARGET_ANY) is never gated, whatever the reaction:
        assert_eq!(faction_target_violation(false, false, true, true), None);
    }
