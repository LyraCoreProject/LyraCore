...
mod gc;
mod gm;
// Module-private DOOR/BUTTON collision-mesh store (issue #112, #103's dynamic GameObject
// collision epic): the import reducers `--go-models` calls. Consumption (the state-gated ray
// merge) is #113's `game_go_collider` registry.
mod go_model;
