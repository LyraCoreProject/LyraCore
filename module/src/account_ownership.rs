...
fn deadline(ctx: &ReducerContext) -> Result<i64, String> {
    now(ctx).checked_add(CLAIM_MICROS).ok_or_else(|| "Account claim deadline overflow".into())
}
...
pub fn claim_account(
    ...
    let account = ctx.db.game_account().id().find(account_id).ok_or("no such Account")?;
    if account.banned {
    ...
    let generation = match &prior {
        Some(row) if row.request_nonce == request_nonce => {
            return if !row.closed && row.expires_micros > now(ctx) && row.character_guid == character_guid {
                Ok(())
            } else {
                Err(STALE.into())
            };
        ...
        Some(row) => row.generation.checked_add(1).ok_or("Account generation exhausted")?,
        None => 1,
    };
    let row = AccountClaim {
        account_id, generation, request_nonce, character_guid,
        expires_micros: deadline(ctx)?, closed: false,
    };
    if prior.is_some() { claims.account_id().update(row); } else { claims.insert(row); }
    Ok(())
}
...
pub fn renew_account_claim(ctx: &ReducerContext, token: WorldSessionToken) -> Result<(), String> {
    ...
    if !same(token, row.generation, row.request_nonce) || row.closed || row.expires_micros <= now(ctx) {
...
pub fn release_account_claim(ctx: &ReducerContext, token: WorldSessionToken) -> Result<(), String> {
    crate::helpers::require_operator(ctx)?;
    if let Some(mut row) = ctx.db.game_account_claim().account_id().find(token.account_id) {
...
pub fn fence_account(
    ...
    crate::helpers::require_operator(ctx)?;
    if token.generation == 0 || token.request_nonce == 0 || character_guid == 0 || expires_micros <= now(ctx) {
        return Err(STALE.into());
    }
    if let Some(character) = ctx.db.game_character().guid().find(character_guid) {
        if ctx.db.game_account().id().find(character.account_id).is_none_or(|account| account.username != account_name) {
    ...
    if let Some(row) = &prior {
        if row.account_name != account_name { return Err("Account fence name changed".into()); }
        if token.generation < row.generation {
            return Err(STALE.into());
        }
        if token.generation == row.generation {
            return if same(token, row.generation, row.request_nonce) && !row.closed
                && row.expires_micros > now(ctx) && row.character_guid == character_guid && row.account_name == account_name {
                Ok(())
            } else {
                Err(STALE.into())
            };
    ...
    let row = AccountFence {
        account_id: token.account_id, account_name, generation: token.generation, request_nonce: token.request_nonce,
        character_guid, expires_micros, closed: false,
    };
    if prior.is_some() { fences.account_id().update(row); } else { fences.insert(row); }
    Ok(())
}
...
pub fn close_account_fence(ctx: &ReducerContext, token: WorldSessionToken) -> Result<(), String> {
    crate::helpers::require_operator(ctx)?;
    if let Some(mut row) = ctx.db.game_account_fence().account_id().find(token.account_id) {
...
fn remove_account_characters(ctx: &ReducerContext, account_name: &str) {
    if let Some(account) = ctx.db.game_account().username().find(account_name.to_owned()) {
        let guids: Vec<_> = ctx.db.game_character().by_account().filter(account.id)
            .map(|character| character.guid).collect();
        for guid in guids { remove_character(ctx, guid, account_name); }
    }
}
...
fn remove_character(ctx: &ReducerContext, guid: u64, account_name: &str) {
    if let Some(entity) = ctx.db.game_world_entity().guid().find(guid) {
        if ctx.db.game_account().id().find(entity.account_id).is_none_or(|account| account.username != account_name) {
            return;
        }
...
pub(crate) fn require_actor(ctx: &ReducerContext, actor: SessionActor) -> Result<u64, String> {
    if let Some(token) = actor.ownership {
        if let Some(row) = ctx.db.game_account_fence().account_id().find(token.account_id) {
            if same(token, row.generation, row.request_nonce) && !row.closed
                && row.expires_micros > now(ctx) && row.character_guid == actor.guid {
                return Ok(actor.guid);
            }
        } else if let Some(row) = ctx.db.game_account_claim().account_id().find(token.account_id) {
            if same(token, row.generation, row.request_nonce) && !row.closed
                && row.expires_micros > now(ctx) && row.character_guid == actor.guid {
                return Ok(actor.guid);
            }
    ...
    let owned = ctx.db.game_account_fence().by_character().filter(actor.guid).any(|row| !row.closed)
        || ctx.db.game_account_claim().by_character().filter(actor.guid).any(|row| !row.closed);
    if owned { Err(STALE.into()) } else { Ok(actor.guid) }
}
...
pub(crate) fn reap_account_fences(ctx: &ReducerContext) {
    let expired: Vec<_> = ctx.db.game_account_fence().by_closed().filter(false)
        .filter(|row| row.expires_micros <= now(ctx)).collect();
