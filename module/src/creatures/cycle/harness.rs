...
struct Scenario {
    ...
    /// Positions read BEFORE any pass ran (the sweep) and by the first pass AFTER spline advance.
    /// Together they show whether a range read sees the rendered position or the leg start.
    seen_by_sweep: RefCell<Vec<(u64, Point)>>,
    seen_after_advance: RefCell<Vec<(u64, Point)>>,
...
// Not migrated yet: the cycle SEQUENCES these, the harness cannot run them. Patrol is the first
// pass after spline advance, so it stands in for every later range read until a real pass migrates.
impl LegacyPasses for Scenario {
    fn legacy_patrol(&mut self, _active: &HashSet<u64>) -> usize {
        *self.seen_after_advance.borrow_mut() = self.snapshot();
        0
    }
    ...
    fn legacy_return(&mut self, _active: &HashSet<u64>, _tick_secs: f32) -> usize {
        0
    }
    fn legacy_wander(&mut self, _active: &HashSet<u64>) -> usize {
        0
    }
...
fn a_later_pass_reads_the_advanced_position_not_the_leg_start() {
    ...
    assert_eq!(
        w.seen_after_advance.borrow().as_slice(),
        [(WOLF, p(5.0, 0.0, 10.0))],
        "every pass after advance must read the rendered position, or the whole cycle decides \
         against a place the creature is not"
    );
}
