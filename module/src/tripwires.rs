...
mod character_fence_tripwire {
    ...
    const WHITELIST: &[(&str, usize, &str)] = &[
        ...
        ("module/src/guild.rs", 2, "guild-chat's `push_guild_chat_event` reads a recipient's owner_identity to ADDRESS a guild-chat event row and the sender's name for the row's `other_name` (the same push_whisper/push_event verdict — no write to the character); the ZERO/empty fallback is the intended answer on realm-core, where no character row exists at all"),
